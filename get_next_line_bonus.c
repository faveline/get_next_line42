...
/*   Updated: 2023/11/03 10:26:32 by faveline         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_creat_new(char *next_line, char *buff, int flag, int save)
{
	...
	next_line = ft_cat_free_bonus(next_line, next_line_tmp, k, flag);
	return (next_line);
}
...
char	*get_next_line(int fd)
{
	...
	next_line = ft_read_next(fd, buff[fd], size);
	ft_modif_buff_bonus(buff[fd], BUFFER_SIZE);
	return (next_line);
}
